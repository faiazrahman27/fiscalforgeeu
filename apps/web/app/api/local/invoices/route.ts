...
export async function GET(request: NextRequest) {
  ...
  try {
    const apiResponse = await fetch(`${apiBaseUrl}/api/v1/invoices${queryString}`, {
      method: "GET",
    ...
    return buildLocalApiProxyError(
      "LOCAL_PRODUCTION_INVOICES_PROXY_ERROR",
...
export async function POST(request: NextRequest) {
  ...
  try {
    const apiResponse = await fetch(`${apiBaseUrl}/api/v1/invoices`, {
      method: "POST",
      headers: await buildLocalApiHeaders({
    ...
    return buildLocalApiProxyError(
      "LOCAL_PRODUCTION_INVOICES_PROXY_ERROR",
      "Could not create the production invoice through the local API proxy.",
      503
    );
  }
}
