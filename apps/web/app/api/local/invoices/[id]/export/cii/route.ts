...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    const apiResponse = await fetch(
      `${apiBaseUrl}/api/v1/invoices/${encodeURIComponent(id)}/export/cii`,
      {
        method: "POST",
    ...
    return buildLocalApiProxyError(
      "LOCAL_PRODUCTION_INVOICE_CII_EXPORT_PROXY_ERROR",
      "Could not export the production invoice as technical CII XML through the local API proxy.",
      503
    );
  }
}
