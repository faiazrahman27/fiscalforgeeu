...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    const apiResponse = await fetch(
      `${apiBaseUrl}/api/v1/invoices/${encodeURIComponent(id)}/export/ubl`,
      {
        method: "POST",
    ...
    return buildLocalApiProxyError(
      "LOCAL_PRODUCTION_INVOICE_UBL_EXPORT_PROXY_ERROR",
      "Could not export the production invoice through the local API proxy.",
      503
    );
  }
}
