...
export async function POST(request: Request) {
  ...
  try {
    const apiResponse = await fetch(`${apiBaseUrl}/api/v1/invoices/export/cii`, {
      method: "POST",
    ...
    return buildLocalApiProxyError(
      "LOCAL_INVOICE_CII_EXPORT_PROXY_ERROR",
      "Could not reach the Invoice Lantern API for CII export readiness. Make sure apps/api is running on port 4000.",
      503
    );
  }
}
