...
export async function POST(request: Request) {
  ...
  try {
    const apiResponse = await fetch(`${apiBaseUrl}/api/v1/invoices/export/ubl`, {
      method: "POST",
    ...
    return buildLocalApiProxyError(
      "LOCAL_INVOICE_UBL_EXPORT_PROXY_ERROR",
      "Could not reach the Invoice Lantern API for UBL export readiness. Make sure apps/api is running on port 4000.",
      503
    );
  }
}
