...
test("UBL export returns SHA-256 metadata and persists metadata-only export records", async (t) => {
  ...
  assert.doesNotMatch(xml as string, /Generated by Invoice Lantern/i);
  assert.doesNotMatch(xml as string, /not Peppol-certified/i);
  assert.equal(
    xmlSha256,
  ...
  assert.match(disclaimer as string, /Peppol/i);
