...
test("generates UBL XML with expanded canonical fields", () => {
...
test("does not inject the Invoice Lantern legal disclaimer as invoice content", () => {
  const xml = buildUblInvoiceXml();

  assert.doesNotMatch(xml, /Generated by Invoice Lantern/i);
  assert.doesNotMatch(xml, /not official validation/i);
  assert.doesNotMatch(xml, /not Peppol-certified/i);
  assert.doesNotMatch(xml, /<cbc:Note>Generated by Invoice Lantern/i);
  assert.match(
    xml,
    /<cac:PaymentTerms>\s*<cbc:Note>Due within 30 days for technical sandbox testing\.<\/cbc:Note>\s*<\/cac:PaymentTerms>/
  );
});
...
test("UBL XSD adapter returns not_configured without artefact configuration", async () => {
  ...
  assert.equal(typeof result.summary.disclaimer, "string");
  assert.match(result.summary.disclaimer as string, /not official validation/i);
  assert.equal(
    result.findings.some(
      (finding) =>
...
test("UBL XSD adapter blocks external schema locations before validation", async () => {
  const tempRoot = await mkdtemp(join(tmpdir(), "invoice-lantern-ubl-xsd-"));
  const maindocPath = join(tempRoot, "xsd", "maindoc");
  const invoiceXsdPath = join(maindocPath, "UBL-Invoice-2.1.xsd");
  ...
  try {
    ...
    await writeFile(
      ...
      `<?xml version="1.0" encoding="UTF-8"?>
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  <xs:include schemaLocation="https://example.test/External-Test-Only-Dependency.xsd"/>
  <xs:element name="Invoice" type="InvoiceType"/>
</xs:schema>`,
      "utf8"
    );
...
<Invoice><ID>INV-XSD-EXTERNAL-001</ID></Invoice>`,
      rootElement: "Invoice",
      documentType: "invoice",
      artifactConfig: {
        rootDir: tempRoot,
    ...
    assert.equal(result.markedValid, false);
    assert.equal(result.artifactInfo.configured, true);
    assert.equal(result.artifactInfo.dependencyGraph.status, "external_reference_blocked");
    assert.equal(result.summary.reason, "external_schema_location_not_supported");
    assert.equal(result.findings[0]?.code, "UBL_XSD_VALIDATOR_ERROR");
...
<Invoice><ID>INV-XSD-VALID-001</ID></Invoice>`;
  ...
  try {
    ...
    assert.match(result.summary.disclaimer as string, /technical schema check only/i);
...
test("UBL XSD adapter rejects unsafe XML before local XSD validation", async () => {
  const tempRoot = await mkdtemp(join(tmpdir(), "invoice-lantern-ubl-xsd-"));
...
<Invoice><ID>&xxe;</ID></Invoice>`;
  ...
  try {
    ...
    assert.equal(result.summary.reason, "xml_doctype_blocked");
    assert.deepEqual(result.summary.xmlSafety, {
      safe: false,
      code: "XML_DOCTYPE_BLOCKED",
      byteLength: Buffer.byteLength(unsafeXml, "utf8"),
      maxDepth: 150
    });
